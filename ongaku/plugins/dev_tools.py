...
from ongaku.tools import shell


# Run shell commands
async def run_cmd(ongaku, message):
...
async def live_shell(ongaku, message):
    cmd = message.input.strip()
...
async def executor(ongaku, message):
    ...
    try:
        ...
        exec(f"async def _exec(ongaku, message):\n    {formatted_code}")
        func_out = await asyncio.Task(
            locals()["_exec"](ongaku, message), name=reply.task_id
        )
    except asyncio.exceptions.CancelledError:
        return await reply.edit("`Cancelled....`")
...
async def loader(ongaku, message):
    if (
        not message.replied
...
@ongaku.add_cmd(cmd=["cancel", "c"])
async def cancel_task(ongaku, message):
